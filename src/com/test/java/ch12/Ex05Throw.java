    ...
    public static void login(User user){
        try {
            if(!DB_ID.equals(user.getId())) {
                throw new IDMismathchException("잘못된 아이디를 입력하셨습니다.");
            } else if (!DB_PW.equals(user.getPassword())) {
                throw new PasswordMismatchException("잘못된 패스워드를 입력하셨습니다.");
            } else {
                user.setLogin(true);
            }
        } catch (IDMismathchException e) {
...
class IDMismathchException extends Exception {
    public IDMismathchException(String message){
        super(message);
    }
}
